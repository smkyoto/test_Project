...
//using Microsoft.DirectX.AudioVideoPlayback;// for DirectX
...
public class Shooting : Form
{
	...
	private Image 味方機体画像, 味方ミサイル画像, 敵機画像, 敵ミサイル画像;
    private int   点数=0;
    //private Audio 爆発音,相手発射音,味方発射音;// for DirectX
	...
	// private void アイコン配列表示(Graphics g, List A)   //for .NET 2.0
	{
		...
		{
			アイコン M = (アイコン)A[i]; g.DrawImage(M.Image, M.X, M.Y);
		}
	}
	...
	{
		if(random.Next(100) < 5) // 敵機生成・ミサイル発射数を調整するにはここを変更
		{
            // 相手発射音.Stop();相手発射音.Play();// for DirectX
            int index = random.Next(敵機配列.Count);
			アイコン 敵機 = (アイコン)敵機配列[index];
			アイコン ミサイル = new アイコン(
				敵ミサイル画像,	敵機.X + 敵機.Width / 2 - 敵機.Width / 2,
				敵機.Y + 敵機.Height
				);
			敵ミサイル配列.Add(ミサイル);
		}
	...
	private void 敵ミサイル移動()
	{
		...
		{
			アイコン ミサイル = (アイコン)敵ミサイル配列[i];ミサイル.Y += 5;
	...
	private void 敵機画面外削除()
	{
		...
		{
			アイコン 敵機 = (アイコン)敵機配列[i]; 敵機.Y += 4;
			if(i % 2 == 0) 敵機.X += 2;	else 敵機.X -= 2;
	...
	private void 着弾()
	{
        //爆発音.Play();// for DirectX
		味方機体.Image= Image.FromFile("着弾.gif");
		Invalidate(); timer.Stop();
	}
	private void 敵の当たり判定()
	{
		for(int i = 0 ; i < 敵機配列.Count ; i++) 
		{
			...
			for(int j = 0 ; j < 味方ミサイル配列.Count ; j++) 
			{
				...
				{
                    //味方発射音 = new Audio("味方発射.wav"); 味方発射音.Play();// for DirectX
					点数=点数+1; 味方ミサイル配列.RemoveAt(j); 敵機配列.RemoveAt(i);
					break;
				}
			}
		}
	...
	{
		敵の生成();	敵のミサイル発射();	敵ミサイル移動();味方ミサイル移動();
		敵機画面外削除(); 敵の当たり判定();	敵ミサイルとの接触判定();
		Invalidate();
	}
	...
	public bool 発射() 
	{
        ...
        //味方発射音.Stop(); 味方発射音.Play();// for DirectX
	...
	private void 開始()
	{
		...
		画面    = new Bitmap(640, 480);	random  = new Random();
        //爆発音 = new Audio("爆発音.wav");// for DirectX
        //相手発射音 = new Audio("相手発射.wav");// for DirectX
        //味方発射音 = new Audio("味方発射.wav");// for DirectX
	...
	{
        this.SuspendLayout(); // Shooting
        this.AutoScaleBaseSize = new System.Drawing.Size(5, 12);
        this.ClientSize = new System.Drawing.Size(292, 266);
        this.Name = "Shooting"; this.ResumeLayout(false);
